...
from .. import utils
from .. import _timing
def rle_to_mask(image_shape, seg):
    mask = np.zeros(image_shape, dtype=np.uint8)
    rle_string = seg["counts"].split()
    for i in range(0, len(rle_string), 2):
        start_pixel = int(rle_string[i]) 
        length = int(rle_string[i + 1])
        start_col,start_row = divmod(start_pixel - 1, image_shape[0])  
        end_col,end_row  = divmod(start_pixel + length - 1, image_shape[0])  
        mask[start_row:end_row + 1, start_col:end_col + 1] = 255
    return mask

class MouseYouTubeVIS(_BaseDataset):
    """Dataset class for YouTubeVIS tracking"""
    @staticmethod
    def get_default_dataset_config(TRACKERS_TO_EVAL=None, SPLIT_TO_EVAL=None):
        """Default class config values"""
        code_path = utils.get_code_path()
        
        if SPLIT_TO_EVAL is None:
            SPLIT_TO_EVAL = 'test'
        default_config = {
            ...
            'TRACKERS_TO_EVAL': TRACKERS_TO_EVAL,  # Filenames of trackers to eval
            'CLASSES_TO_EVAL': None,  # Classes to eval (if None, all classes)
            'SPLIT_TO_EVAL': SPLIT_TO_EVAL,  # Valid: 'test', 'test_wo17'
            'PRINT_CONFIG': True,  # Whether to print current config
            'OUTPUT_SUB_FOLDER': '',  # Output files are saved in OUTPUT_FOLDER/tracker_name/OUTPUT_SUB_FOLDER
            'TRACKER_SUB_FOLDER': "test",  # Tracker files are in TRACKER_FOLDER/tracker_name/TRACKER_SUB_FOLDER
    ...
    def __init__(self, config=None):
        ...
        self.gt_fol = self.config['GT_FOLDER'] + self.config['SPLIT_TO_EVAL']
        ...
        self.seq_list = [vid['id'] for vid in self.gt_data['videos']]
        self.seq_name_to_seq_id = {vid['id']: vid['id'] for vid in self.gt_data['videos']}
        self.seq_lengths = {vid['id']:vid["length"]  for vid in self.gt_data['videos']}
    ...
    def _prepare_gt_annotations(self):
        ...
        for track in self.gt_data['annotations']:
            ...
            for i, seg in enumerate(track['segmentations']):
                if seg:
                    mask = rle_to_mask((h,w),seg)
                    mask_encoded = mask_utils.encode(np.asfortranarray(mask))
                    mask_encoded["counts"] = mask_encoded["counts"].decode('utf-8')
                    track['segmentations'][i] = mask_encoded
